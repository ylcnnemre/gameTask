...
const Header = () => {
  ...
  const [mobileHeaderShow, setMobileHeaderShow] = useState<boolean>(false);
  ...
  const menu = (
    <Menu
      style={{ padding: "0 10px", zIndex: "9999999" }}
      onClick={({ key }) => {
        setLangueage(key);
      }}
      items={[
    ...
    <header className="header">
      ...
      <div className="header_button_menu">
        <div className="dropdown_container">
          ...
          <AiOutlineBars
            className="bars_icon"
            onClick={() => setMobileHeaderShow(!mobileHeaderShow)}
          />
        </div>

        {auth ? (
          <Link to={""} className="logout_button" onClick={() => logOut()}>
            {t("logout")}
          </Link>
        ) : (
      ...
      <MobileHeader
        mobileHeaderShow={mobileHeaderShow}
        setMobileHeaderShow={setMobileHeaderShow}
      />
    </header>
  );
};
