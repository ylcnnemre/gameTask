...
const Login = () => {
  ...
  const { setAuth }: IContextType = useContext(Context);
  ...
  const confirmLogin = (): void => {
    ...
    if (userData !== null) {
      const StorageLoginInfo: { email: string; password: string } =
        JSON.parse(userData);

      if (
        loginInfo.email == StorageLoginInfo.email &&
        loginInfo.password == StorageLoginInfo.password
      ) {
        setAuth(true);
    ...
    <section className="login">
      <div className="login_container">
        <div className="login_wrapper">
          <h1 className="login_wrapper_logo">PT</h1>
          <h2 className="title">{t("signmessage")}</h2>
